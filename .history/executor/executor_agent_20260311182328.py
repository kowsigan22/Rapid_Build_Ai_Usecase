...
def execution_agent(state: BuildState) -> BuildState:
    try:
        ...
        if execution_descriptor["application_type"] != "frontend":
        ...
        if is_long_running and execution_descriptor["application_type"] == "web-service":
            if run_command.startswith("uvicorn"):
                print("long_running")
            ...
            elif run_command.startswith("python"):
                print("long_running")
                run_command = run_command.replace("python", "", 1).strip()
                process = subprocess.Popen(
                    [venv_python] + run_command.split(),
                    cwd=app_root,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
                    text=True
            ...
            try:
                ...
                state["status"] = "EXECUTION_FAILED"
                state["error"] = "npm is not installed or not found in the system PATH."
                return state
            print("pass 1")
            try:
                subprocess.run(["node", "--version"], shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            except subprocess.CalledProcessError:
                state["status"] = "EXECUTION_FAILED"
                state["error"] = "node is not installed or not found in the system PATH."
                return state
            print("pass 2")
            ...
            if install_result.returncode != 0:
                state["status"] = "EXECUTION_FAILED"
                state["error"] = f"Frontend dependency installation failed:\n{install_result.stderr}"
                return state

            build_cmd = execution_descriptor.get("build")
            print("pass 4")
            if build_cmd and execution_descriptor["framework"] != "react":
                build_result = subprocess.run(
                    build_cmd,
                    cwd=working_dir,
                    shell=True,
...
while True:
    ...
    if line:
        ...
        for pattern in interactive_patterns:
            if pattern in lower_line:
                process.kill()
                state["status"] = "EXECUTION_FAILED"
                state["error"] = "".join(logs)
                return state

        # Detect known react issue
        if "unable to detect target browsers" in lower_line:
            process.kill()
            state["status"] = "EXECUTION_FAILED"
            state["error"] = "".join(logs)
            return state
        ...
        # Detect success
        if "compiled successfully" in lower_line or "starting the development server" in lower_line:
            break
    ...
    if time.time() - start_time > timeout_seconds:
        process.kill()
        state["status"] = "EXECUTION_FAILED"
        state["error"] = "Application startup timed out.\n" + "".join(logs)
        return state
