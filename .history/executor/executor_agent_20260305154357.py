...
def execution_agent(state: BuildState) -> BuildState:
    ...
    #         f.write("\n".join(requirements))
        ...
        if is_long_running & execution_descriptor["app_type"] == "web-service":
            print("long_running")
        ...
        elif execution_descriptor["app_type"] == "batch-job":
            result = subprocess.run(
                run_command.split(),
                cwd=app_root,
                capture_output=True,
        ...
        elif execution_descriptor["app_type"] == "frontend":
            ...
            ports = execution_descriptor.get("ports", [])
            exposed_urls = [f"http://localhost:{p}" for p in ports]

            state["exposed_urls"] = [
                {"method": "GET", "url": url} for url in exposed_urls
            ]
