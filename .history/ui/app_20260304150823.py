...
if run_button and user_prompt.strip():
    ...
    if "final_state" not in st.session_state and final_state:
...
# SHOW STORED PIDS
# -----------------------------
if "port_pids" in st.session_state:

    for pid in st.session_state["port_pids"]:
        st.markdown(f"### 🧩 Application using port {st.session_state.get('current_port')}")
        st.write(f"**PID:** {pid}")
