...
if st.button("Check Port"):
    if port_input.strip() == "":
        ...
        matching = [line for line in lines if f":{port_input}" in line]
        ...
        if matching:
            for line in matching:
                ...
                st.success(f"Port {port_input} is used by PID: {pid}")

                if st.button(f"❌ Kill PID {pid}"):
                    subprocess.run(
                        ["taskkill", "/PID", pid, "/F"],
                        capture_output=True
                    )
            ...
            st.info(f"Port {port_input} is free.")
elif run_button:
