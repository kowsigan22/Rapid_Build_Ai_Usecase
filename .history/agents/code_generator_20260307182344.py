...
def repair_llm_json(raw_output: str):
    print("repair check 1")
    if not raw_output:
        raise ValueError("LLM output empty")
    print("repair check 2")

    text = raw_output.strip()
    print("repair check 3")

    # remove wrapping triple quotes if present
    if text.startswith('"""') and text.endswith('"""'):
        text = text[3:-3].strip()
    print("repair check 4")
    ...
    # fix decorator splits like:
    # @app.get
    # ("/")
    text = re.sub(r'@(\w+)\.(\w+)\s*\n\s*\(', r'@\1.\2(', text)

    # remove newline before parentheses
    text = re.sub(r'\n\s*\(', '(', text)

    # remove newline inside URLs
    text = re.sub(r'http://[^\s"]+\n', lambda m: m.group(0).replace("\n", ""), text)
    print("repair check 5")

    # escape newlines inside JSON strings
    def fix_string(match):
        print("repair check 7")

        content = match.group(1)
        content = content.replace("\n", "\\n")
        return '"' + content + '"'
    print("repair check 6")

    text = re.sub(r'"([^"\\]*(?:\\.[^"\\]*)*)"', fix_string, text)
    print("RAW LLM OUTPUT 2>>>", repr(text))
...
def code_generator_agent(state: BuildState) -> BuildState:
    ...
    try:
        ...
        print("RAW LLM OUTPUT 1 >>>", repr(code))
        try:
payload = json.loads(code)        except json.JSONDecodeError:
            # Re-prompt the LLM with stricter instructions
            return None
        ...
        if "generated_files" not in payload:
            ...
            raise ValueError("LLM output missing 'generated_files' or Invalid LLM JSON: missing generated_files")
